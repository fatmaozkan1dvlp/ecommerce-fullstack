...
const Urunler = () => {
    ...
    const filtreliUrunler = useMemo(() => {
        ...
        if (seciliKategori !== "Hepsi") {
            sonuc = sonuc.filter(u => (u.kategoriAd || u.KategoriAd) === seciliKategori);
        }
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                    <div className="xl:col-span-2 flex flex-wrap gap-2 items-center bg-gray-50 dark:bg-gray-900/40 p-3 rounded-2xl border dark:border-gray-800">
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-[11px] font-black tracking-wider transition-all ${seciliKategori === "Hepsi" ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border dark:border-gray-700'}`}
                        >
                            HEPSİ
                        </button>
                        {kategoriler.map(k => (
                            <button
                                key={k.id || k.ID}
                                onClick={() => setSeciliKategori(k.ad || k.Ad)}
                                className={`px-4 py-2 rounded-xl text-[11px] font-black tracking-wider transition-all ${seciliKategori === (k.ad || k.Ad) ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border dark:border-gray-700'}`}
                            >
                                        ...
                                        <div key={u.id || u.ID} className={`grid grid-cols-1 md:grid-cols-12 gap-4 items-center px-6 py-5 md:px-8 hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-all group ${u.stok === 0 ? 'opacity-70 grayscale-[0.5]' : ''}`}>
                                            ...
                                            <div className="col-span-1 md:col-span-5 flex items-center gap-4">
                                                <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-gray-700 flex-shrink-0 overflow-hidden border-2 border-white dark:border-gray-600 shadow-sm">
                                                    {u.galeri && u.galeri.length > 0 ? (
                                                        <img
                                                            ...
                                                            onError={(e) => { e.target.src = "https://via.placeholder.com/150?text=Yok"; }}
                                                        />
                                                    ) : (
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-[10px] text-gray-400 mt-1 md:hidden uppercase font-bold tracking-widest">{u.kategoriAd || "Genel"}</span>
                                                </div>
                                            </div>
